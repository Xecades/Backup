...
Typing.prototype.init = function () {
    if (!!this.head) {
        $("." + this.className).append(
            "<span class='typing-head'>" + this.head + "</span>"
        );
    }
    $("." + this.className).append("<span class='typing-container'></span>");
    $("." + this.className).append(
        "<span class='cursor' style='color: " +
            this.cursorColor +
            "'>" +
            this.cursorChar +
            "</span>"
    ...
    if (!!this.foot) {
        $("." + this.className).append(
            "<span class='typing-foot'>" + this.foot + "</span>"
        );
    }
    if (this.fade == true) {
        $(".cursor").css(
            "transition",
            "opacity " + this.cursorSpeed / 2000 + "s"
        );
    }
    var show = true;
    var This = this;
...
Typing.prototype.addTask = function (val) {
    ...
    var charInterval = setInterval(function () {
        $("." + This.className + " .typing-container").append(
            "<span>" + This.stringName.charAt(count) + "</span>"
        );
        count++;
        if (count == length) {
...
Typing.prototype.deleteTask = function (val) {
    ...
    var count = 0;
    var show = true;
    var allCharLength = $("." + This.className + " .typing-container")[0]
        .children.length;
    if (!val == true) {
        val = allCharLength;
    }
    if (val > allCharLength) {
        console.error(
            "The delete function's param must less than String's length"
        );
    } else {
        var charInterval = setInterval(function () {
            $(
                "." +
                    This.className +
                    " .typing-container span:nth-last-child(1)"
            ).remove();
            count++;
            if (count == val) {
                clearInterval(charInterval);
