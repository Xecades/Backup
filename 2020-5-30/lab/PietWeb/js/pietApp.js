...
function createImage() {
	document.getElementById("btn").innerHTML = "Computing...";
	...
	base64 = 'data:image/png;base64,' + p.getBase64();
	document.getElementById("btn").innerHTML = "generate";
	if (generated) {
		document.getElementById("image").className = 'result small';
		setTimeout(function () {
			document.getElementById("image").innerHTML = '<img style="width:10vw;" src="' + base64 + '">';
		}, 500);
		setTimeout('document.getElementById("image").className = "result big";', 500);
	} else {
		document.getElementById("image").innerHTML = '<img style="width:10vw;" src="' + base64 + '">';
		document.getElementById("image").className = 'result big';
	}
	generated = true;
}
...
function view() {
	if (window.navigator.msSaveOrOpenBlob) {
		...
		window.navigator.msSaveOrOpenBlob(blob, 'GeneratedPiet' + '.' + 'png');
	} else {
		// 这里就按照chrome等新版浏览器来处理
		const a = document.createElement('a');
		a.href = base64;
		a.setAttribute('download', 'GeneratedPiet');
		a.click();
	}
}
